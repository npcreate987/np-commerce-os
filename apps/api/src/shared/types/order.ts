...
export const addressSchema = z.object({
  ...
  /// optional geo (ใช้กับ Local Commerce / Express delivery)
...
export const orderSchema = z.object({
  ...
  /// Phase 5: ส่วนลดรวม (coupon + loyalty redeem) — optional for backward compat
  discountCents: z.number().int().nonnegative().default(0).optional(),
  totalCents: z.number().int(),
  shippingAddress: addressSchema,
  carrierCode: z.string().nullable().optional(),
  /// Phase 5: code คูปองที่ใช้ (สำหรับแสดงผล)
...
export const createCheckoutSchema = z.object({
  ...
  /// affiliate / creator link code (resolved server-side)
  affiliateCode: z.string().min(1).max(40).optional(),
  /// Phase 5: คูปองส่วนลด (resolved server-side)
  couponCode: z.string().min(3).max(32).optional(),
  /// Phase 5: แต้มที่ใช้แลกเป็นเครดิตส่วนลด (1 แต้ม = 1 บาท)
